...
export class Character {
  ...
  update(): this {
    switch (this.computedStyle.textOrientation) {
      ...
      case 'mixed':
      default:
        this.verticalOrientation = U_R_RE.test(this.content)
          ? TR_UR_RE.test(this.content) ? 'Tu' : 'U'
          : TR_UR_RE.test(this.content) ? 'Tr' : 'R'
        break
    }
    this.isPunctuation = PUNCTUATION_RE.test(this.content)
    this.isEOL = EOL_RE.test(this.content)
    return this
  }
  ...
  clone(): Character {
    const c = new Character(this.content, this.parent)
    c.contentBox = this.contentBox.clone()
    c.glyphBox = this.glyphBox.clone()
    return c
  }
}
