...
function parseCharsPerRepeat(size: HighlightSize, fontSize: number, width: number): number {
  if (size === 'cover') {
    return 0
  }
    ...
    if (size.endsWith('%')) {
      ...
      return Math.ceil(rate * width) / fontSize
    }
...
function parseStrokeWidthScale(strokeWidth: HighlightStrokeWidth, fontSize: number, width: number): number {
